...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  console.log("[Cleanup Artwork Connections] Starting cleanup of orphaned artwork connections")
  ...
  try {
    ...
    console.log(`[Cleanup] Found ${artworks.length} artworks to check`)
    ...
    console.log(`[Cleanup] Found ${existingProductIds.size} existing products`)
    ...
    for (const artwork of artworks) {
      ...
      if (currentProductIds.length === 0) {
        console.log(`[Cleanup] Artwork ${artwork.id} (${artwork.title}) has no product connections`)
        continue
      }
      ...
      if (orphanedCount > 0) {
        console.log(`[Cleanup] Artwork ${artwork.id} (${artwork.title}):`)
        ...
        console.log(`[Cleanup] Artwork ${artwork.id} (${artwork.title}) has all valid connections (${validProductIds.length})`)
      }
    }
    ...
    console.error("[Cleanup] Error cleaning up artwork connections:", error)
    res.status(500).json({
      success: false,
