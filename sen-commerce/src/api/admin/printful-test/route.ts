...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  console.log("[Printful Test] Testing Printful order creation directly")
  ...
  try {
    ...
    console.log("[Printful Test] Creating Printful order with data:", JSON.stringify(testOrderData, null, 2))
    
    const printfulOrder = await printfulOrderService.createOrder(testOrderData)
    
    if (printfulOrder) {
      console.log("[Printful Test] Printful order created successfully:", printfulOrder.id)
      res.json({
        success: true,
      ...
      console.log("[Printful Test] Failed to create Printful order")
      res.json({
        success: false,
        message: "Failed to create Printful order - check logs for details"
      })
    }
